...
## 2. 게임 진행
- [x] 각 라운드마다 증가되는 카운트 `BridgeGame`에 필드로 추가 (default: 1)
### a. 이동할 칸 입력 받기
- 조건
- [x] `U`(위 칸)와 `D`(아래 칸) 중 하나의 문자만 입력 가능
- 예외
- [x] 올바른 값이 아니면 예외 처리 후 다시 입력 받기
### b. 이동 적용
- [x] `BridgeGame` 내 `move()`를 이용해 이동 가능한지 판별 
### c. 진행 상황 출력
- 조건
...
- [x] 현재까지 건넌 다리를 모두 출력
- [x] 마지막에 빈 줄 출력
### d. 실패 시, 재시작/종료 여부 입력 받기
- 조건
- [x] `R`(재시작)과 `Q`(종료) 중 하나의 문자를 입력 가능
- [x] 재시작할 경우, 기존 진행 상황 초기화
- [x] 재시작할 경우, 시도 횟수 증가
- 예외
- [x] 올바른 값이 아니면 예외 처리 후 다시 입력 받기
### e. 게임 성공 시, 성공 처리
- [x] `BridgeGame`의 won 필드 true로 변경
## 3. 게임 종료
### a. 최종 게임 결과 출력
- [x] 마지막 진행 상황을 그대로 출력
### b. 게임 성공 여부 출력
- [x] 마지막 게임 결과에 따라 `성공`, `실패` 출력
### c. 총 시도한 횟수 출력
- [x] 첫 시도부터 게임을 종료할 때까지 시도한 횟수 출력
  <br/>
